...
import { Animal, DashboardStats, Distributor, Meal, NextMealInfo } from '@/types';
import {
  animalsApi,
  distributorApi,
  mealsApi,
...
export default function HomeScreen() {
  ...
  const { user } = useAuth();
  const { notifyMealDistributed } = useNotifications();
  const [animals, setAnimals] = useState<Animal[]>([]);
  const [distributorStatus, setDistributorStatus] = useState<Distributor[]>([]);
  const [nextMealsByDistributor, setNextMealsByDistributor] = useState<NextMealInfo[]>([]);
  const [stats, setStats] = useState<DashboardStats>({ consumption: 0, regularity: 0 });
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (mealsData && mealsData.length > 0 && statusData && statusData.length > 0) {
        const enabledMeals = mealsData.filter((meal: Meal) => meal.enabled);
        const now = new Date();
        const currentTimeStr = `${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}`;

        // Créer une map des prochains repas par mangoire
        const nextMealsByDist: NextMealInfo[] = [];

        statusData.forEach((distributor: Distributor) => {
          // Trouver tous les repas pour cette mangoire
          const mealsForDistributor = enabledMeals
            .filter((meal: Meal) => meal.distributorId === distributor.id)
      ...
      if (statsData && statsData.length > 0) {
        ...
        setStats({
          consumption: todayStats.totalConsumed,
      ...
      setStats({ consumption: 0, regularity: 0 });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDistributeNow = async (mealId: number) => {
      ...
      { text: 'Annuler', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            ...
            Alert.alert('Erreur', (error as Error).message || 'Erreur lors de la distribution');
          }
        },
      },
    ]);
              ...
              <View key={distributor.id} style={styles.distributorCard}>
                <View style={styles.distributorHeader}>
                  <Text style={styles.distributorLabel}>
                    {distributor.name || `Mangeoire ${distributor.id}`}
                  </Text>
                  <View
                    style={[
        ...
        <View style={styles.statsContainer}>
          <View style={styles.statCard}>
            ...
            <Text style={styles.statValue}>
              {stats.consumption ? `${stats.consumption}g` : '0g'}
            </Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>🎯</Text>
            <Text style={styles.statLabel}>Régularité</Text>
            <Text style={styles.statValue}>{stats.regularity ? `${stats.regularity}%` : '0%'}</Text>
          </View>
        </View>
      </View>
