...
export default function AnimalManagementScreen() {
  ...
  const [animalBreeds, setAnimalBreeds] = useState<AnimalBreedOption[]>([]);
  const [filteredBreeds, setFilteredBreeds] = useState<AnimalBreedOption[]>([]);
  ...
  const loadAllData = async () => {
    try {
      ...
      if (typesList && typesList.length > 0) {
        ...
        const firstBreeds = (breedsList || []).filter(
          (b: AnimalBreedOption) => b.type === typesList[0].label,
        );
  ...
  const handleEditAnimal = async () => {
    ...
    try {
      await animalsApi.update(editingAnimal.id as number, {
        name: formData.name.trim(),
        type: formData.type,
        breed: formData.breed,
        gender: formData.gender,
    ...
    Alert.alert(
      ...
      [
        { text: 'Annuler', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await animalsApi.delete(animal.id as number);
              loadAllData();
  ...
  const openEditModal = (animal: Animal) => {
    ...
    setFormData({
      name: animal.name,
      type: animal.type || 'Chien',
      breed: animal.breed || 'Labrador Retriever',
      gender: animal.gender || 'Mâle',
      age: animal.age?.toString() || '0',
      ageUnit: animal.ageUnit || 'Années',
      weight: animal.weight?.toString() || '0',
